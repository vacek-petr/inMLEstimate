...
%       interpretation of CG, see (Meurant,Tichy 2023, second inequality in (3.5) with updating formula for a coefficient (3.3)]),
...
switch(stoppingCriterionName)
    ...
    case 'GR'
        ...
        gammaGR = 1/smallestEigenvalueLowerBound;
        UpperBoundGRSq = gammaGR*rrNew;
        if (UpperBoundGRSq < 0)
...
while(~stoppingCriterionSatisfied)    
    ...
    switch(stoppingCriterionName)
        ...
        case 'GR'
            gammaGR = (gammaGR-alpha)/(smallestEigenvalueLowerBound*(gammaGR-alpha) + delta);
            UpperBoundGRSq = gammaGR*rrNew;
            if (UpperBoundGRSq < 0)
                error('GR bound failed.');
            end
